...
\end{document}ift 的伪逆做的事情是：把 $[A; R_2]$ 做 Smith Normal Form（$R_2 = 2I$ 是余域关系矩阵），对角线 $d_i \neq 0$ 的位置取 $1/d_i$。当 $d_i = 2$ 时，伪逆给出 $1/2$，这就是分数 $-1/2$ 的来源。

关键问题：Z-lift 的伪逆 \textbf{不检查方程是否有解}。即使方程在 $\mathbb{Z}_2$ 上无解（$cp$ 不在 $d_1$ 的像中），Z-lift 仍然会返回一个"最佳拟合"的解，其中包含分数。这个分数解是\textbf{无意义的}——它甚至不满足方程 $x \cdot A \equiv cp \pmod{2}$。

\subsection{后果链}

\begin{enumerate}
\item $d_1$ 矩阵有 2-torsion → GF(2) 秩从 4 降到 2
\item gen1 的 CF obstruction $cp$ 在列 4, 7 有奇数分量 → 不在 $d_1$ 的 GF(2) 像中
\item \texttt{SptSetZLMapInverse} 不检查可解性 → 返回分数 $n = [\ldots, -1/2]$
\item 分数 $n$ 传入 \texttt{SptSetSolveCocycleEq} → 生成分数 $n_3$ 函数
\item 分数 $n_3$ 进入 O5 公式 → \texttt{phase mod 2} 崩溃（\texttt{ModRat}）
\item 或者：公式跑完但结果不对 → dO5 ≠ 0 → \texttt{ASSERTION FAILURE: top layer is not a cocycle}
\end{enumerate}

\subsection{关键开放问题}

\begin{enumerate}
\item \textbf{gen1 真的是 Majorana Chain 吗？}如果用户的 C++ 代码用的是 gen4 或其他能解的生成元，那 GAP 对 gen1 报错是正确的行为——gen1 确实有不可消除的 CF obstruction。
\item \textbf{如果 gen1 确实应该可解}，那么问题可能在于：
  \begin{itemize}
  \item InstallCoboundary(2,2,2) 公式在 Z-lift 下给出了错误的 cp（虽然我们证明了公式 mod 2 是不变的，但也许某些中间步骤有问题）
  \item CoboundarySL 的层索引或系数作用有误
  \item 或者谱序列的 $E_5^{2,2}$ 模块计算有误，gen1 不应该出现在 $E_5$ 页
  \end{itemize}
\item \textbf{2D 为什么不出问题？}需要检查 2D 情况下 $d_1$ 矩阵的 SNF 是否有 2-torsion。如果没有，那 2D 的方程总是可解的。
\end{enumerate}
