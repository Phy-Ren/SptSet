...
\item \sout{测试 C3v s12} ——已完成，见 \S\ref{sec:c3v-s12-failure}
\item 测试 SG\#30/36/41 等 ASSERTION 警告群，确认修复后群结构正确
\item 与 C++ 程序的结果对比
\item 如果多群验证通过，将修复合并回生产线
\end{itemize}

\section{S4 s12/EZ 诊断实验：第四、第五个成功}
\label{sec:s4-success}

S4 点群（$|G|=4$，SG\#81）的 s12 和 EZ 两种模式均\textbf{零报错通过}。

\subsection{S4 s12 结果}
...
层 & $E_2$ & $E_3$ & $E_4$ & $E_5$ & $E_\infty$ \\
\hline
$p{+}ip$ & $\mathbb{Z}_2$ & $0$ & --- & --- & $0$ \\
MC & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & --- & $\mathbb{Z}_2$ \\
CF & $\mathbb{Z}_2$ & $0$ & --- & --- & $0$ \\
Bosonic & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ \\
\hline
\end{tabular}
\end{center}

群结构 $\mathbb{Z}_4$。
Phase B 进度：MC 的 1 个 generator 与 Bosonic 的 1 个 generator
做模扩展（torsion=2），得到 $\mathbb{Z}_4$。
总耗时约 14 分钟。
...
层 & $E_2$ & $E_3$ & $E_4$ & $E_5$ & $E_\infty$ \\
\hline
$p{+}ip$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ \\
MC & $\mathbb{Z}_2$ & $0$ & --- & --- & $0$ \\
CF & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & --- & $\mathbb{Z}_2$ \\
Bosonic & $\mathbb{Z}_2$ & $0$ & --- & --- & $0$ \\
\hline
\end{tabular}
\end{center}

群结构 $\mathbb{Z}_4$。
注意 EZ 模式下 $p{+}ip$ 存活到 $E_\infty$（与 s12 不同），
MC 和 Bosonic 反而被杀死，CF 存活。
$p{+}ip$ 到 CF 的 extension 给出 $\mathbb{Z}_4$。
总耗时约 22 分钟。

两个模式均\textbf{零诊断触发}。

\section{C3v s12 诊断实验：崩溃——$p{+}ip$ 的 $d_4$ 微分产生分数}
\label{sec:c3v-s12-failure}

C3v s12 是 debug 分支上的\textbf{首个失败实验}，
也是唯一一个 $p{+}ip$ 层存活到 $E_5$ 的测试用例。

\subsection{实验配置}

与 C4v s12 完全相同的模板：
\begin{itemize}
\item \textbf{群}：C3v（$|G|=6$，SG\#156），s12 模式
\item \textbf{脚本}：\texttt{debug/exp\_c3v\_s12\_diag.g}
\item \textbf{Resolution}：\texttt{ResolutionFiniteGroup(PG, 9)}，维度 $1,2,3,4,5,6,7,8,9$
\end{itemize}
...
层 & $E_2$ & $E_3$ & $E_4$ & $E_5$ & $E_\infty$ \\
\hline
$p{+}ip$ ($q=3$) & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$
      & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ \\
MC ($q=2$) & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & --- & $\mathbb{Z}_2$ \\
CF ($q=1$) & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & --- & $\mathbb{Z}_2$ \\
Bosonic ($q=0$) & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$
      & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ \\
\hline
\end{tabular}
\end{center}

关键特征：\textbf{$p{+}ip$ 一路存活到 $E_\infty = \mathbb{Z}_2$}。
这是与之前所有成功实验的本质区别——
C4v/S4 s12 的 $p{+}ip$ 在 $E_3$ 就被 $d_2$ 杀死，
SG\#30/22 EZ 的 $p{+}ip$ 从 $E_2$ 就为零。

Phase A 耗时约 1.8 小时，其中 $E^{1,3}_5$ 最重（约 1.6 小时）。

\subsection{最早的错误：$d_4$ 微分出现分数}

\textbf{Phase A 期间}就已经出现了第一个诊断触发（第 78 行）：
\begin{verbatim}
!! DIAG BuildDerivative(r=4,p=1,q=3,gen=1/1): opr FRACTION,
   indices=[ 3, 5, 6 ] values=[ 1/2, 3/4, 3/4 ]
\end{verbatim}

这是什么意思？用人话说：

\begin{enumerate}
\item 谱序列的每一页 $E_r$ 有一个微分 $d_r$，
      把 $(p,q)$ 处的东西射到 $(p+r, q-r+1)$。
\item $d_4$ 从 $p{+}ip$ 在 $(1,3)$ 射到 $(1+4, 3-4+1) = (5, 0)$。
      注意 $(5,0)$ 已经\textbf{超出物理上的四层}
      （$p{+}ip$ 在 $p=1$，MC 在 $p=2$，CF 在 $p=3$，Bosonic 在 $p=4$）。
\item 计算 $d_4$ 需要把 $p{+}ip$ 的 cocycle 一路 purify 上去，
      解出它在 $p=2,3,4,5$ 各层的对应 cochain。
      而解 $p=5$ 层需要用到 \texttt{addTwister} 公式——
      这些公式编码了 $p{+}ip$ 层的 stacking 规则。
\item \textbf{但是 $d_3$ 和 $d_4$ 涉及的 $p{+}ip$ 高阶 stacking 规则，
      物理上我们确实不知道}（代码里被设成零或不完整），
      所以算出来有 $\frac{1}{2}$、$\frac{3}{4}$ 这些分数。
\end{enumerate}

尽管如此，$E^{1,3}_5$ 的分类结果（$\mathbb{Z}_2$）本身可能是对的，
因为 $d_4$ 的像为零（被 Smith 正规形约化掉了）。
但这些分数已经污染了内部 cache。

\subsection{Phase B 崩溃链}

Phase B 在构建 $p{+}ip$ 的 class 时（\texttt{ComponentEx(1,3) class 1/1}），
触发了连锁诊断：

\begin{enumerate}
\item \textbf{PurifyClass（第 139 行）}：purification 把 $p{+}ip$ 的 cocycle 展开到
      $p=5$（Bosonic 以上）层，发现分数值 $[-\frac{1}{2}, -\frac{3}{4}, -\frac{3}{4}]$，
      对应索引 $[3,5,6]$——与 Phase A 的 $d_4$ 错误完全对应。
      这就是旧生产线上的 ``top layer is not a cocycle'' 错误。
\item \textbf{ZLMapInverse（第 141 行）}：后续解逆映射也产生分数
      $[-\frac{15}{4}, -\frac{11}{4}, -\frac{3}{4}]$，连锁反应。
\item \textbf{最终崩溃（第 163 行）}：$p{+}ip$ 的 class 被分数污染后，
      \texttt{ClassToLeadingVector} 返回了错误维度的向量
      （长度 3 而不是 1），extension 矩阵赋值时维度不匹配崩溃：
\begin{verbatim}
Error, List Assignments: <rhss> must have the same length as <poss>
  (lengths are 3 and 1)
  Rmat[j]{[ r1 + 1 .. r ]} := vjnf
\end{verbatim}
\end{enumerate}

\subsection{根因：物理限制，不是代码 bug}

\begin{center}
\begin{tabular}{l|cc}
\hline
实验 & $p{+}ip$ 命运 & 需要算 $d_4$ 吗？ \\
\hline
SG\#30 EZ & $E_2$ 就为零 & 不需要 \\
SG\#22 EZ & $E_2$ 就为零 & 不需要 \\
C4v s12 & $E_3 = 0$（$d_2$ 杀死） & 不需要 \\
S4 s12 & $E_3 = 0$（$d_2$ 杀死） & 不需要 \\
S4 EZ & 存活，但 $d_4$ 的目标层 trivial & 无害 \\
\textbf{C3v s12} & \textbf{存活到 $E_5$} & \textbf{必须算 → 出错} \\
\hline
\end{tabular}
\end{center}

所有之前成功的实验，$p{+}ip$ 要么一开始就 trivial，
要么在 $E_3$ 被 $d_2$ 杀死，$d_4$ 根本不会被调用。
C3v s12 是少数 $p{+}ip$ 存活到高阶的困难用例，
$d_4$ 计算需要我们\textbf{不知道的物理公式}（$p{+}ip$ 层的高阶 stacking 规则），
因此产生分数。

\textbf{这个问题与我们修复的 Component/Derivative 统一无关。}
那个修复解决的是所有 $p{+}ip$ trivial 或早期被杀死的情况。
C3v s12 的问题是更根本的：物理上缺少 $p{+}ip$ 的 $d_3/d_4$ stacking 规则。

\subsection{待做}

\begin{itemize}
\item 对 C3v s12，考虑跳过 $p{+}ip$ 层只算 MC/CF/Bosonic
      （\texttt{pRange=[2,3,4]}），
      这应该能避开 $d_4$ 问题
\item 补充 $p{+}ip$ 的高阶 stacking 公式（如果物理上已知）
\item 继续测试更多群验证已有修复的稳健性
\item 将修复合并回生产线
\end{itemize}

\section{SG\#42 EZ 诊断实验：攻克第二个 ModRat 崩溃群}
\label{sec:sg42-ez-success}

SG\#42 (Fmm2) 是生产线上唯二 ModRat 崩溃群中的第二个（第一个 SG\#22 已在
\S\ref{sec:sg22-ez-success} 中成功）。
生产线上 Phase B 每次都在 \texttt{ext layer 2 (p=2)} 崩溃（ModRat 错误），
本次是该群的\textbf{首次完整成功}。

\subsection{实验配置}

\begin{itemize}
\item \textbf{群}：SG\#42 (Fmm2)
\item \textbf{模式}：EZ（$w = 0$，spinless）
\item \textbf{脚本}：\texttt{debug/exp\_sg42\_ez\_diag.g}
\item \textbf{Resolution}：\texttt{Resolution3DSpaceGroup(fSG, 9)}，
      维度 $1, 5, 12, 20, 28, 36, 44, 52, 60$（与 SG\#22 完全相同）
\item \textbf{并行}：\texttt{SPTSET\_PARALLEL\_JOBS=10}
\end{itemize}
...
层 & $E_2$ & $E_3$ & $E_4$ & $E_5$ & $E_\infty$ \\
\hline
$p{+}ip$ & $\mathbb{Z}_2$ & $0$ & 跳过 & 跳过 & $0$ \\
MC & $\mathbb{Z}_2^7$ & $\mathbb{Z}_2$ & $\mathbb{Z}_2$ & --- & $\mathbb{Z}_2$ \\
CF & $\mathbb{Z}_2^9$ & $0$ & 跳过 & --- & $0$ \\
Bosonic & $\mathbb{Z}_2^6$ & $\mathbb{Z}_2^2$ & $\mathbb{Z}_2^2$ & $\mathbb{Z}_2^2$ & $\mathbb{Z}_2^2$ \\
\hline
\end{tabular}
\end{center}

$p{+}ip$ 在 $d_2$ 被杀死，CF 在 $E_3$ 被杀死。
只有 MC（$\mathbb{Z}_2$）和 Bosonic（$\mathbb{Z}_2^2$）存活。
Phase A 耗时约 4 小时，其中 $E^{2,2}_4$（MC 层 $d_3$）最重（约 3.7 小时）。
...
\begin{verbatim}
  ComponentEx(2,2): 1 generators
  ComponentEx(2,2) class 1/1... done [32544792 ms]   <-- 9 小时
  ComponentEx(4,0): 2 generators
  ComponentEx(4,0) class 1/2... done [232 ms]
  ComponentEx(4,0) class 2/2... done [232 ms]
  >> ModExt: layer [ 2, 3 ] gen 1/1 torsion=2       <-- 旧代码在这里崩溃！
  extension done [33676320 ms]                        <-- 现在成功！
\end{verbatim}

MC 的 $\mathbb{Z}_2$ 与 Bosonic 的 $\mathbb{Z}_2^2$ 做扩展，
torsion 不变（仍为 2），最终是直积 $\mathbb{Z}_2 \times \mathbb{Z}_2^2 = \mathbb{Z}_2^3$。

Phase B 耗时约 18.4 小时（主要在 ComponentEx(2,2) class 1 和 extension 上）。
总计约 22.3 小时。

\subsection{诊断结果}

\textbf{零诊断触发。}搜索 log 中 \texttt{!! DIAG}：零匹配。

\subsection{两个 ModRat 崩溃群的对比}
...
& SG\#22 EZ & SG\#42 EZ \\
\hline
Resolution 维度 & $1,5,12,20,\ldots,60$ & $1,5,12,20,\ldots,60$ \\
$p{+}ip$ & $E_2 = 0$ & $E_3 = 0$（$d_2$ 杀死） \\
$E_\infty$ & MC=$\mathbb{Z}_2$, Bos=$\mathbb{Z}_2^8$
           & MC=$\mathbb{Z}_2$, Bos=$\mathbb{Z}_2^2$ \\
群结构 & $\mathbb{Z}_2^9$ & $\mathbb{Z}_2^3$ \\
诊断警告 & 0 & 0 \\
旧生产线 & ModRat $\times 5$ & ModRat $\times ?$ \\
\hline
\end{tabular}
\end{center}

\textbf{至此，生产线上所有 ModRat 崩溃群（SG\#22 和 SG\#42）
均已被 debug 版本成功攻克。}
...
\hline
实验 & 类型 & 旧生产线 & debug 结果 & 诊断 \\
\hline
SG\#30 EZ & 空间群 & ASSERTION 警告 & $\mathbb{Z}_2 \times \mathbb{Z}_4 \times \mathbb{Z}_8$ & 0 \\
C4v s12 & 点群 & ASSERTION 警告 & $\mathbb{Z}_2^4$ & 0 \\
SG\#22 EZ & 空间群 & \textbf{ModRat 崩溃} & $\mathbb{Z}_2^9$ & 0 \\
S4 s12 & 点群 & 正常 & $\mathbb{Z}_4$ & 0 \\
S4 EZ & 点群 & 正常 & $\mathbb{Z}_4$ & 0 \\
SG\#42 EZ & 空间群 & \textbf{ModRat 崩溃} & $\mathbb{Z}_2^3$ & 0 \\
C3v s12 & 点群 & GCD\_INT 崩溃 & 崩溃（$p{+}ip$ $d_4$ 分数） & 3 \\
\hline
\end{tabular}
\end{center}

7 个实验中 6 个\textbf{零报错成功}，
唯一失败的 C3v s12 是因为 $p{+}ip$ 存活到 $E_5$，
触发了物理上未知的 $d_4$ stacking 公式，\textbf{与 Component/Derivative 修复无关}。

\textbf{结论}：统一使用旧微分系统（Component/Derivative）的修复对所有
$p{+}ip$ trivial 或早期被杀死的群\textbf{完全有效}，
可以考虑合并回生产线。

\section{SG\#36 EZ 诊断实验：快速通过}
\label{sec:sg36-ez-success}

SG\#36 (Cmc$2_1$) 是生产线上有 ASSERTION 警告的空间群之一。

Phase A：$p{+}ip$ 在 $d_2$ 被杀死，CF 在 $E_4$ 存活。
$E_\infty$：MC=$\mathbb{Z}_2$，CF=$\mathbb{Z}_2$，Bosonic=$0$。
Phase B：$\mathbb{Z}_2^2$。零诊断触发。总耗时约 3 小时。

\section{$\mathbb{Z}_4^{f,T}$ s12 诊断实验：理论验证与新 bug 修复}
\label{sec:z4ft-s12}

这是一个\textbf{极其重要}的实验。
$\mathbb{Z}_4^{f,T}$ 是最简单的非平凡费米子 SPT 对称群，
理论结果完全已知（见 \texttt{3+1D\_Stacking.tex} 第 1271--1362 行），
可以精确验证程序的正确性。
更重要的是，这是\textbf{第一个所有四层（含 $p{+}ip$）都存活到 $E_\infty$} 的实验，
暴露并修复了一个新的代码 bug。

\subsection{物理背景}

\begin{itemize}
\item $G_f = \mathbb{Z}_4^{f,T}$：费米子对称群，
      $T^2 = P_f$（时间反演的平方等于费米子宇称）
\item $G_b = \mathbb{Z}_2^T = \{e, T\}$：玻色子商群
\item $s_1(T) = 1$：$T$ 是反幺正算符
\item $\omega_2(T, T) = 1$：非平凡的 $H^2(\mathbb{Z}_2, \mathbb{Z}_2)$ cocycle，
      编码了 $T^2 = P_f$ 的扩展 $1 \to \mathbb{Z}_2^f \to \mathbb{Z}_4 \to \mathbb{Z}_2 \to 1$
\end{itemize}

\subsection{GAP 中的群构造}

\begin{verbatim}
G := Group([(1,2)]);;               % G_b = Z_2, 生成元 T = (1,2)
f := GroupHomomorphismByImagesNC(G, GL(1, Integers),
     [(1,2)], [ [[-1]] ]);;         % s_1(T) = 1, 即 T 反幺正
w := function(g1, g2)               % omega_2: 非平凡 2-cocycle
    if 1^g1 = 2 then                %   检查 g1 = T
        return (1 - (g2^f)[1][1])/2; %   检查 g2 是否也反幺正
    else return 0; fi; end;;         %   w(T,T) = 1, 其余 = 0
ss := FermionSPTSpecSeq(R, f, w);;  % s12 模式
\end{verbatim}

与已有例子 \texttt{examples/ap\_z4\_z2t.g}（第 26--35 行的 $H$ 子群部分）
使用完全相同的公式和命名约定。
...
\textbf{所有四层 $E_\infty = \mathbb{Z}_2$}，所有微分 $d_r = 0$。
与理论预期完全一致。
零诊断触发。Phase A 耗时 114 ms（$\mathbb{Z}_2$ 群非常小）。

\subsection{Phase B：新 bug 的发现与修复}

\subsubsection{不含 $p{+}ip$ 的结果}

$\texttt{pRange} = [2,3,4]$（MC + CF + Bosonic）：
\[
\boxed{\mathbb{Z}_8}
\]
与理论预期完全一致。扩展链：
\[
\mathbb{Z}_2 \xrightarrow{\text{torsion}=2} \mathbb{Z}_4
\xrightarrow{\text{torsion}=4} \mathbb{Z}_8
\]
耗时 3.8 秒。

\subsubsection{含 $p{+}ip$ 的崩溃与修复}

$\texttt{pRange} = [1,2,3,4]$ 首次运行时\textbf{崩溃}：
\begin{verbatim}
Error, List Assignments: <rhss> must have the same length as <poss>
  (lengths are 3 and 1)
  Rmat[j]{[ r1 + 1 .. r ]} := vjnf
  at lib/ss_module.gi:244
\end{verbatim}

\textbf{根本原因}：\texttt{SptSetSpecSeqModuleExtension} 中的维度不匹配 bug。

模块有两个``空间''：
\begin{itemize}
\item \textbf{生成元空间}（$r$ 维）：Smith 标准形后的独立生成元个数
\item \textbf{嵌入空间}（$n$ 维）：底层 cochain 向量的维数
\end{itemize}

对于刚创建的单层模块，$r = n$。
但模块扩展函数内部会对 $M_1$ 做规范化（Smith 标准形），
这会减少 $r$（去掉 torsion$=1$ 的平凡生成元）但 $n$ 不变。
然而 \texttt{vector\_embedings} 字段\textbf{没有被同步更新}，
仍然指向旧的 $r$ 维空间（现在已是 $n$ 维嵌入空间）。

当 \texttt{ClassToLeadingVector(M\_rest, ...)} 被调用时，
返回的向量维度是 $n = 3$（嵌入空间），
但扩展函数期望的是 $r = 1$（生成元空间）。

\textbf{为什么以前没触发}：
在之前所有成功实验中，$p{+}ip$ 都在 Phase A 被杀死（$E_\infty = 0$），
代码直接跳过了最后的扩展步骤（\texttt{if IsZero(Epip) then return M\_rest}）。
$\mathbb{Z}_4^{f,T}$ 是\textbf{第一个} $p{+}ip$ 存活到 $E_\infty$
且参与 Phase B 群扩展的实验。

\textbf{修复}（\texttt{lib/ss\_module.gi}，
\texttt{SptSetSpecSeqModuleClassToLeadingVector} 函数）：
当返回向量维度 $\neq$ 模块生成元个数时，
用 \texttt{projection} 矩阵从嵌入空间投影到生成元空间：
\begin{verbatim}
result := CanonicalElm(M!.components[p], v) * M!.vector_embedings[p];
r := SptSetNumberOfGenerators(M);
if Length(result) <> r then
    result := result * M!.projection;
fi;
\end{verbatim}
对于 $r = n$ 的正常模块（之前所有实验），条件不触发，行为不变。
对于 $r < n$ 的规范化模块，正确转换维度。

\subsubsection{修复后含 $p{+}ip$ 的结果}

$\texttt{pRange} = [1,2,3,4]$（全部四层）：
\[
\boxed{\mathbb{Z}_{16}}
\]
\textbf{与理论预期完全一致。}

\subsection{$p{+}ip$ 扩展的计算路径}
...
机制 & 物理含义 & 用途 \\
\hline
\texttt{InstallCoboundary} & obstruction function & \textbf{Phase B 扩展}（已知） \\
\texttt{InstallAddTwister} & stacking rule & Phase A 微分（部分未知） \\
\hline
\end{tabular}
\end{center}

Phase B 的扩展计算通过 purification 机制工作：
\begin{enumerate}
\item 取 $p{+}ip$ 生成元 $n_1 = m_1$，乘以 torsion 2：$2m_1$
\item $2m_1 = \mathrm{d}_{s_1}(m_0)$，其中 $m_0 = -1$（coboundary）
\item Gauge 掉 $p{+}ip$ 层时，MC 层通过 \texttt{Coboundary(2,0,3)} 获得修正：
      $m_0() \cdot \omega_2(g_1, g_2) = -\omega_2(g_1, g_2)$
\item 在 $\mathbb{Z}_2$ 系数下 $-\omega_2 = \omega_2 = m_1^2$，
      即 MC 层的非平凡生成元
\item 因此扩展关系：$2 \times (\text{p+ip gen}) = 1 \times (\text{MC gen})$
\item $\mathbb{Z}_2$ 通过此关系与 $\mathbb{Z}_8$ 扩展 $\to \mathbb{Z}_{16}$
\end{enumerate}

\textbf{关键结论}：
$p{+}ip$ 的 \texttt{AddTwister(1,3)} 和 \texttt{AddTwister(2,2)}
虽然设为零（物理公式未知），但它们只影响 Phase A 的高阶微分 $d_r$，
\textbf{不影响} Phase B 的群扩展。
Phase B 用的是 \texttt{Coboundary} 公式，这些\textbf{完全已知}。

\subsection{与 C3v 崩溃的对比}

\begin{center}
\begin{tabular}{l|cc}
\hline
& $\mathbb{Z}_4^{f,T}$ & C3v s12 \\
\hline
$p{+}ip$ $E_\infty$ & $\mathbb{Z}_2$（存活） & $\mathbb{Z}_2$（存活） \\
所有 $d_r$ & 全部 $= 0$ & $d_4 \neq 0$（需要未知公式） \\
Phase A & 成功 & 崩溃（分数） \\
Phase B & 成功（修复后） & 未到达 \\
根本原因 & 维度不匹配 bug（已修复） & 物理公式缺失（未解决） \\
\hline
\end{tabular}
\end{center}
...
S4 EZ & 点群 & 正常 & $\mathbb{Z}_4$ & 0 \\
SG\#42 EZ & 空间群 & \textbf{ModRat 崩溃} & $\mathbb{Z}_2^3$ & 0 \\
SG\#36 EZ & 空间群 & ASSERTION 警告 & $\mathbb{Z}_2^2$ & 0 \\
$\mathbb{Z}_4^{f,T}$ s12 & 有限群 & 维度 bug & $\mathbb{Z}_{16}$（含 $p{+}ip$） & 0 \\
 & & & $\mathbb{Z}_8$（不含 $p{+}ip$） & 0 \\
C3v s12 & 点群 & GCD\_INT 崩溃 & 崩溃（$p{+}ip$ $d_4$ 分数） & 3 \\
\hline
\end{tabular}
\end{center}

9 个实验中 8 个\textbf{零报错成功}。
唯一失败的 C3v s12 是 Phase A 的物理公式缺失，与代码 bug 无关。

已修复的 bug 共两类：
\begin{enumerate}
\item \textbf{Component/Derivative 不一致}（之前修复）：
      影响所有群的 Phase A/B，导致 ModRat 和 ASSERTION 错误
\item \textbf{模块扩展维度不匹配}（本次修复）：
      影响 $p{+}ip$ 存活的群的 Phase B 扩展，
      \texttt{ClassToLeadingVector} 返回嵌入空间维度而非生成元空间维度
\end{enumerate}
