# 3D 点群 FSPT STC 批量计算 — s12 模式 (spin-1/2, with p+ip)
# 基于 debug 线 lib/ 源代码，遵守实验铁律
# 遍历 3D 点群，跳过 C3v（已知 bug: rational propagates into SNF）
# 输出群结构（对应 PhysRevX.15.031029 Table 3 左列，with omega2）
#
# 运行: cd ~/gap-debug && gap -o 50G -l ... pkg/SptSet/examples/fspt_3d_s12_batch.g
...
Print("================================================================\n");;
Print("3D Point Group FSPT STC — s12 mode (spin-1/2, with p+ip)\n");;
...
_skip_list := ["C3v"];;
Print("Skip list: ", _skip_list, "\n");;
Print("Total point groups: ", Length(PtGrpNames), "\n");;
Print("============================================================\n\n");;

_count := 0;;
for i in [1..Length(PtGrpNames)] do
    if PtGrpNames[i] in _skip_list then
...
Print("============================================================\n");;
Print("ALL DONE: ", _count, " point groups (s12 mode), ",
      Length(_skip_list), " skipped\n");;
SptSetPrintStats();;
FORCE_QUIT_GAP(0);;
