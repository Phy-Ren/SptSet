...
# Compare code's s1-quarter terms vs locked formula's s1-half term
# on a concrete anti-unitary example (G=Z2xZ2, s1=alpha, n3 generator).
...
# s1 = alpha: first generator anti-unitary
gens := GeneratorsOfGroup(G);;
auAlpha := GroupHomomorphismByImagesNC(G, GL(1, Integers), [gens[1], gens[2]], [[[-1]], [[1]]]);;
s := g -> (1-(g^auAlpha)[1][1])/2;;
smod := g -> s(g) mod 2;;

# n3: canonical cocycle of H^3(Z2^2,Z2), take bar class [1,0,0,0]
n3vec := List([1..Dimension(R)(3)], i -> 0);; n3vec[1] := 1;;
n3m := SptSetMapToBarCocycle(brMap, 3, SptSetCoefficientZn(2, triv), n3vec);;
