InstallMethod(FermionEZSPTSpecSeq,
  ...
  function(R, auMap)
    ...
    # 4+1D p+ip -> Majorana-chain obstruction for omega2=0:
    #   dn2 = 0 on the E_2 page, and
    #   dn3 = n2 cup n2 + s1 cup (n2 cup_1 n2)  (mod 2).
    ...
    # 4+1D Majorana-chain -> complex-fermion obstruction for omega2=0:
    #   dn4 = n3 cup_1 n3 + s1 cup (n3 cup_2 n3)  (mod 2).
    ...
    # EZ has omega2=0.  The n4-dependent complex-fermion terms are installed
    # separately in InstallCoboundary(ss, 2, 4, 1).
    #
    # DOMAIN: n3 must be a Z2 cocycle (dn3 = 0 mod 2).  See the full
    # (non-EZ) ss_fermion.gi for a detailed explanation.
    SptSetInstallCoboundary(ss, 3, 3, 2,
    function(n3, dn3)
      local n3m, w0, smod, beta, betam,
      ...
      # Canonical cocycle representative.  The spectral sequence machinery
      # may pass a cochain whose values off the bar-resolution basis do not
      # form a mod-2 cocycle (e.g. after purification stacks a non-cocycle
      # into this layer); the O6^gamma formula evaluates cup and surjection
      # products on arbitrary tuples, and beta = d(tilde n3)/2 below is then
      # half-integer valued (ModRat crash).  Round-trip through the bar
      # resolution to replace n3m by the canonical equivariant cocycle of the
      # same class; this does not change the E_3 class of the result.
      ...
      quarterOmegaBeta := Cup0@(2, 4, coeffZ, w0, beta);
    ...
    # replaces Cup1/Cup2 and addTwister(5,0) Cup3 with direct Z2 expansion
    # O5c = n3 cup1 n3 + dn3 cup2 n3
    # O5cgamma = dn3 cup3 dn3  (was generated by addTwister(5,0), now included here)
    SptSetInstallCoboundary(ss, 2, 3, 1, function(n3, dn3)
      return function(g1, g2, g3, g4, g5)
        local phase;
        # O5c: n3 cup1 n3
        phase := n3(g1*g2*g3, g4, g5) * n3(g1, g2, g3)
    ...
    # 4+1D complex-fermion contribution to the bosonic obstruction, omega2=0:
    #   O^c_6 = 1/2[ n4 cup_2 n4 + dn4 cup_3 n4 ],
    #   O^{c gamma}_6 = 1/2[ dn4 cup_4 dn4 ].
    SptSetInstallCoboundary(ss, 2, 4, 1,
    function(n4, dn4)
      local coeff, c6;
      coeff := spectrum[1+1];
      c6 := Cup2@(4, 4, coeff, n4, n4);
    ...
    function(l1, l2)
      ...
      # c3 := AddInhomoCochain@(c3, Cup2@(3, 2, coeff, AddInhomoCochain@(dn21, dn22), N2));
      # c3 := AddInhomoCochain@(c3, Cup1@(2, 2, coeff, m2, N2));
      # c3 := AddInhomoCochain@(c3, Cup2@(2, 3, coeff, m2, InhomoCoboundary@(coeff, N2)));
    ...
    SptSetInstallAddTwister(ss, 1, 3, {l1, l2} -> ZeroCocycle@); # 3+1D p+ip
    SptSetInstallAddTwister(ss, 2, 2, {l1, l2} -> ZeroCocycle@); # 3+1D Majorana chain
    # 3+1D complex fermion
...
InstallGlobalFunction(FermionSPTLayersNoPipVerbose,
function(ss, dim)
  # Print only the triplet (Majorana, complex fermion, bosonic), i.e. q=2,1,0.
  # This is the intended view for the present 4+1D implementation, where the
  # p+ip layer is deliberately excluded.
  local layerNames, p, q, rmax, r, Erpq;
  layerNames := ["Bosonic:", "Complex fermion:", "Majorana:", "p+ip:"];
  for p in [1..(dim+1)] do
    q := dim + 1 - p;
...
InstallGlobalFunction(FermionSPTLayersNoPip,
function(ss, dim)
  local layers, p, q, rmax, E;
  layers := [];
  for p in [1..(dim+1)] do
