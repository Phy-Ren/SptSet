# S4 EZ full diagnostic — 遵守实验铁律
# 从头开始，不加载 checkpoint，走 debug 线 lib/ 源代码
# 模板：SG#30 EZ 成功实验
#
# 背景：S4 点群 (|G|=4, SG#81)，EZ 模式 (w=0, spinless)
# 之前笔记记录 S4 ez 给出 Z4（p+ip 跳过 trivial MC 直接 extend 到 CF）
...
# ============ 铁律 2: 只调用标准函数，建群 + 谱序列 ============
Print("Setting up S4 EZ...\n");;
_t := NanosecondsSinceEpoch();;
PG := PointGroup(SpaceGroupBBNWZ(3, 81));;
...
f := GroupHomomorphismByImagesNC(PG, GL(1, Integers),
    gens, List(gens, x -> [[DeterminantMat(x)]]));;

ss := FermionEZSPTSpecSeq(R, f);;
Print("  Spectral sequence (EZ, spinless) created.\n\n");;

# ============ Phase A: Classification (with zero-module early stop) ============
...
Print("  SptSetSpecSeqResult(ss, 4, [1,2,3,4]) — all layers\n");;
Print("------------------------------------------------------------\n");;
